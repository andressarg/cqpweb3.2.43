...
class DbType
{
	...
	/* specs only used for parlink*/
	//question: do I need this here? is this the right way of doing it?
	public $aligned_sentences = array();
	public $text_id_array = array();
	public $align_att	 = NULL;
	...
	public function __construct($type) 
	{
		...
		case DB_TYPE_PARLINK:
			$this->str = 'parlink';
			$this->aligned_sentences = func_get_arg(1);
			$this->text_id_array = func_get_arg(2);
			$this->align_att = func_get_arg(3);
			break;
...
// TODO replace with a call to the $User object that interrogates privileges.
	...
	} elseif ($commands['notawk']) {
		file_put_contents($tabfile, $commands['notawk']);
	} else {
	...
	/* QUESTION: I want to insert the align_att into the saved_dbs, as a corpus can have more than 1 alignement
	I guess I'd have to make changes at least to the upgraade-database and sql-definitions files.
	I thought it would better to ask you before attempting to make these changes
	*/
	$sql = "INSERT INTO saved_dbs (
			...
			'"  . ($db_type->type == DB_TYPE_COLLOC ? $db_type->colloc_range : '') . "',
			"  . /*($db_type->type == DB_TYPE_PARLINK ? escape_sql($db_type->align_att) : '') . ", */ "
...
// TODO category needs charset/collate
	...
	case DB_TYPE_PARLINK:
		// get the aligend sentences and the text ids
		$aligned_sentences = $db_type->aligned_sentences;
		$text_id_array = $db_type->text_id_array;
		
		$awk_script = false;
		$tabulate_command = false;

		$notawk = NULL;
		// populate table with text_id from the source, ref number and word
		for($i = 0; $i < count($aligned_sentences); $i++)
		{
			$word = explode(" ", $aligned_sentences[$i], -1);

			for($j =0; $j < count($word); $j++)
			{
				$word[$j] = escape_sql($word[$j]);
				$notawk .= $text_id_array[$i] . "\t" . $i . "\t" . $word[$j] . "\n";
...
function check_dblist_parameters($db_type, $cqp_query, $query_scope, $postprocess)
{
	...
	case DB_TYPE_PARLINK:
		// $extra_conditions = " and align_att = '" . escape_sql($db_type->align_att) . "'";
		break;

	...
	if ('count_hits_then_cease' != $program)
	{
		...
		foreach ($marker as $key => $m)
		{
			...
			if ( $n != 0 )
				...
				$final_string .= 'href="concordance.php?qname=' . $cache_record->qname . '&pageNo=' . $n . '&' 
					. print_per_page_for_reinsertion('url', $per_page)
					. (empty($program) ?  '' :  '&program='.$program)
					. url_printget(array(
						array('pageNo', ''), array('qname', ''), ['pp', ''], ['program', '']
						) )
					. '"';
		...
		if ($Corpus->visualise_translate_in_concordance)
			...
			$final_string .= 
				...
				. url_printinputs(array(
					array('viewMode', ''), array('qname', ''), ['pp',''], ['pageNo', ''],  ['program', '']
					))
			...
			$final_string .= "
				<td align=\"center\" width=\"$button_cell_width\" class=\"concordgrey\" nowrap>
					<form action=\"concordance.php\" method=\"get\">
						...
						. print_per_page_for_reinsertion('input', $per_page)
								. url_printinputs( array( ['qname', ''],  ['pageNo', ''], ['pp', ''], ['newPostP', '']
											))
...
' . ( !$Config->hide_experimental_features ? '<option value="Dispersion">Dispersion</option>' . "\n" : '' ) . '
	...
	$final_string .= url_printinputs(array(
		array('redirect', ''), array('qname', ''), ['pp',''], [ 'pageNo', '']
		));
...
function print_alignment_switcher($target_script, $qname, $align_info, $current)
{
	...
	return "<td align=\"center\" width=\"20%\" class=\"concordgrey\" nowrap>"
		...
		. url_printinputs(array(array('showAlign', ''), array('qname', $qname)))
		. '</form>'
		. "<form method=\"post\">"
		. "<input type=\"submit\" name=\"hi_button\" value=\"Highlight\">"
		. "<input type=\"button\" onclick=\"open_parlink_table()\" value=\"Table\">"
		. '</form>'
		. '</td>'
		;
}
...
function print_sort_control($postprocess_string, &$sort_position_out)
{
	global $Corpus;
 ...
 * 
 */
function print_aligned_line($cqp_line, $display_params, $type_significance) 
{
	...
	/* If the highlight option is selected, then show aligend sentences with new spans */
	if(isset($_POST['hi_button']))
		{
		// return "<td class=\"$cell_class\"$colspan$cell_align>oioioioio</td>\n";
		return '<td class="' . $cell_class .'"'. $colspan . $cell_align . '>' . highlight_parlinks($line, $type_significance) . '</td>';
		} else {
...
// 		$translation_content
	...
	// TODO remove any XML from the $node_string and 
...
function print_collocation_warning_cell($query_record)
{
	...
	if (QueryScope::TYPE_WHOLE_CORPUS != $query_record->qscope->type) 
	{
		...
		if (false === $freqtable_record)
			$issue_warning = true;
//TODO if true, also touch the freqtable as we will be using it soon.
	}
		...
		$s = '
			<tr>
				<td class="concorderror" colspan="3">
					...
					of the corpus (' . number_format((float)$tokens_searched) . ' words). No cached frequency data 
					was found and frequency lists for the relevant part of the corpus will have to 
					be compiled in order to provide accurate measures of collocational strength. 

...
{
	$sort_pos_recall = 0;
	$sort_control_row = print_sort_control($cache_record->postprocess, $sort_pos_recall);
...
// create parlink database
if ($show_align)
{
	// get the text ids with the parlinks
	$text_id_array = $cqp->execute("tabulate $qname match text_id");
	// get all sentences
	$all_kwic = $cqp->execute("cat $qname");
	
	// prepare for creation of parlink 
	$aligned_sentences = prepare_parlink($n_key_items, $all_kwic, $alignment_att_to_show);

	// create db if it doesn't already exist
	$type_spec = new DbType(DB_TYPE_PARLINK, $aligned_sentences, $text_id_array, $alignment_att_to_show);
	$db_record = check_dblist_parameters($type_spec, $cache_record->cqp_query, $cache_record->qscope, $postprocess);
	
	if ($db_record === false)
	{
		$dbname = create_db($type_spec, $qname, $cache_record->cqp_query, $cache_record->qscope, $cache_record->postprocess);
	...
	$type_significance = create_parlink_table($dbname, $alignment_att_to_show);
/*  ANDREW: note that calling this func here is prob not the best thing, 
	as it leads to the tabel content being prior to the  table
	you need the data here, but not the copntent-echo. 
	it's likely best to split the function into one that builds the array and one that echoes.
	Call the one that builds here, and other at the end of the <body> -- ie line 1000 in conc-ui.

	QUESTION: wouldn't it be more efficient calling it here, as I wouldn't have to iterate
	trought the result twice?
...
/* column headings */
echo '<tr><th class="concordtable">No</th><th class="concordtable">Text</th><th class="concordtable"'
...
for ( $i = 0, $highlight_check = ($highlight_positions_array !== false) ; $i < $n_key_items ; $i++ )
{
	...
	if ($show_align)
	echo "\n<tr>", print_aligned_line($kwic[++$i], $display_params, $type_significance), "</tr>\n";
